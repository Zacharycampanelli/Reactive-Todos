...
const AuthenticationModal: FC<AuthenticationModalProps> = ({ isOpen, toggleOpen, modalType, setModalType }) => {
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {};

  return (
    <Dialog open={isOpen} onClose={toggleOpen} className="fixed inset-0 z-50 h-screen">
      <DialogPanel className="flex flex-col w-screen h-screen md:w-[70%] md:h-[70%] mx-auto my-auto bg-gradient-to-l from-[#3226AE] to-[#6B1B7E] top-[50%] relative -translate-y-[50%] rounded-lg shadow-lg">
        ...
        <Fieldset className='mx-8'>
          {modalType === 'login' ? <LoginModalBody onSubmit={onSubmit} /> : <RegisterModalBody onSubmit={onSubmit} />}
        </Fieldset>
